              ...
              box(title="Data File Upload", width=12, solidHeader = TRUE, status="primary",
                  ...
                  fileInput("data.file.input",
                            ...
                            accept = c("text/csv",
                                       ...
                                       ".csv")),
                  p("Need some sample data? Click Here!"),
                  downloadButton("downloadData", "Download"),
                  br(),br(),
                  p("Sample response data represents daily customer transactions at a ficticious company from 1/1/14-4/30/14"),
...
server <- function(input, output, session) { 
  ...
  output$DateRange<-renderUI({
    ...
    tagList(
      box(title="Date Ranges", width=12, solidHeader = TRUE, status="primary",
          p("Please select the intervention period (Test data intervention occurs 3/9-4/11)"),
          fluidRow(
            column(width=12,
  ...
  output$downloadData <- downloadHandler(
    ...
    content = function(file) {
      write.csv(test.data(), file, row.names = FALSE)
    }
  )
}
